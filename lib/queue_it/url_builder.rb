...
module QueueIt
  class UrlBuilder
    def self.build_queue_url(customer_id, event_id, target_url = nil)
      "https://queue.iranshao.com?c=#{customer_id}&e=#{event_id}&t=#{URI.encode_www_form_component(target_url)}"
    end

    def self.build_cancel_url(customer_id, event_id, queue_id = nil)
      "https://queue.iranshao.com/cancel.aspx?c=#{customer_id}&e=#{event_id}&q=#{queue_id}"
    end
